...
void copyImgToTex(const cv::Mat& _tex_img, GLuint* texID, double* _twr, double* _thr) {
	cv::Mat tex_img = _tex_img;
	flip(_tex_img,tex_img,0);
	cv::Mat tex_pow2(pow(2.0,ceil(log2(tex_img.rows))),pow(2.0,ceil(log2(tex_img.cols))),CV_8UC3);
	std::cout << tex_pow2.rows <<"x"<<tex_pow2.cols<<std::endl;
	cv::Mat region = tex_pow2(cv::Rect(0,0,tex_img.cols,tex_img.rows));
	if (tex_img.type() == region.type()) {
		tex_img.copyTo(region);
	} else if (tex_img.type() == CV_8UC1) {
		cv::cvtColor(tex_img, region, CV_GRAY2BGR);
	} else {
...
OpenCVGLTexture MakeOpenCVGLTexture(const cv::Mat& _tex_img) {
